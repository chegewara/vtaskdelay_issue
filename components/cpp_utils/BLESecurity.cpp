...
void BLESecurity::setCapability(esp_ble_io_cap_t iocap) {
	m_iocap = iocap;                            // <--- PARAMETER required in function
	esp_ble_gap_set_security_param(ESP_BLE_SM_IOCAP_MODE, &iocap, sizeof(uint8_t));				 // <--- setup esp32 IO capability mode in our application (has display and/or keyboard)
}
/*
 *
